    ...
    <AnimatePresence>
      {chatOpen && (
        <>
          ...
          <motion.div
            ...
            className={`fixed bottom-20 right-4 sm:right-6 z-[100] w-[calc(100vw-2rem)] sm:w-[380px] rounded-2xl overflow-hidden flex flex-col border border-white/[0.08] ${
              isMinimized ? '' : 'h-[min(600px,calc(100vh-120px))]'
            }`}
            style={{ background: '#0b080c' }}
          >
            {/* Header */}
            <div className="flex items-center justify-between px-4 py-3 border-b border-white/[0.06]" style={{ background: '#0e0e1a' }}>
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#c2a4ff] to-[#a78bfa] flex items-center justify-center">
                  <Bot className="w-4 h-4 text-black" />
                </div>
                <div>
                  <h3 className="text-sm font-semibold">👋 Irshad&apos;s AI Clone</h3>
                  <div className="flex items-center gap-1.5">
                    <span className="text-[10px] text-[#c2a4ff]">
                      Ask anything — projects, skills, career! 🚀
                    </span>
                    {apiStatus === 'connected' && (
                      <span className="flex items-center gap-0.5 text-[9px] text-[#c2a4ff] bg-[#c2a4ff]/10 px-1.5 py-0.5 rounded-full">
            ...
            <AnimatePresence>
              {!isMinimized && (
                ...
                >
                  {/* Messages */}
                  <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4 no-scrollbar" style={{ background: '#0a0a12' }}>
                    {chatMessages.length === 0 && (
                      ...
                      >
                        <Sparkles className="w-8 h-8 text-[#c2a4ff]/50 mx-auto mb-3" />
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          {quickActions.map((action) => (
                            <button
                              ...
                              className="text-xs p-2.5 rounded-lg bg-white/5 hover:bg-white/10 text-muted-foreground hover:text-foreground text-left transition-colors border border-white/5"
                            >
                              {action.label}
                            </button>
                          ))}
                      ...
                      >
                        <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 ${
                          ...
                          {msg.role === 'user'
                            ...
                            : <Bot className="w-3.5 h-3.5 text-[#c2a4ff]" />
                          }
                        </div>
                    ...
                    {isTyping && (
                      ...
                      >
                        <div className="w-7 h-7 rounded-full bg-[#c2a4ff]/15 flex items-center justify-center">
                          <Bot className="w-3.5 h-3.5 text-[#c2a4ff]" />
                        </div>
                        <div className="bg-white/5 rounded-lg px-4 py-3 border border-white/5">
                          <div className="flex gap-1">
                            <motion.div
                              ...
                              className="w-1.5 h-1.5 rounded-full bg-[#c2a4ff]/50"
                            />
                            <motion.div
                              animate={{ y: [0, -4, 0] }}
                  ...
                  {chatMessages.length > 0 && chatMessages.length < 3 && (
                    <div className="px-4 pb-2 flex gap-1.5 overflow-x-auto no-scrollbar" style={{ background: '#0a0a12' }}>
                      {quickActions.slice(0, 4).map((action) => (
                        <button
                          key={action.label}
                          onClick={() => sendMessage(action.prompt)}
                          className="text-[10px] px-2.5 py-1.5 rounded-full bg-white/5 hover:bg-[#c2a4ff]/10 text-muted-foreground hover:text-[#c2a4ff] whitespace-nowrap transition-colors border border-white/5"
                        >
                          {action.label}
                        </button>
                      ))}
                  ...
                  {/* Input */}
                  <div className="p-3 border-t border-white/[0.06]" style={{ background: '#0e0e1a' }}>
                    <form
                    ...
                    >
                      ...
                      <Button
                        ...
                        className="bg-[#a855f7] hover:bg-[#c2a4ff] text-white shrink-0"
                      >
      ...
      {!chatOpen && (
        <motion.button
          ...
          className="fixed bottom-6 right-4 sm:right-6 z-[100] w-14 h-14 rounded-full flex items-center justify-center shadow-lg shadow-purple-500/30"
          style={{ background: 'linear-gradient(135deg, #c2a4ff, #a855f7)' }}
        >
